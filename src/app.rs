...
use log::info;
use uefi::{prelude::BootServices, table::boot::ScopedProtocol};
...
impl<'a> IrcClient<'a> {
    ...
    pub fn connect_to_server_and_register(
        ...
        let tcp_protocol = get_tcp_protocol(self.boot_services, &self.tcp_service_binding_protocol);

        let connection = TcpConnection::new(self.boot_services, tcp_protocol, ip_address, port);
