...
impl<'a> App<'a> {
    fn new(
        ...
        let input_box_sizer = move |superview_size: Size| {
            ...
            Rect::from_parts(
                Point::new(0, content_frame.max_y()),
                Size::new(
        ...
        let send_button_sizer = move |superview_size: Size| {
            ...
            Rect::from_parts(
                Point::new(input_box_frame.max_x(), input_box_frame.min_y()),
                Size::new(
    ...
    fn scroll_to_last_visible_line(&self) {
        ...
        *self.content_view.view.view.layer.scroll_offset.borrow_mut() =
            Point::new(0, cursor_pos.y - viewport_height + 30);
    }

    fn scroll_up(&self) {
        let scroll_pos = self.content_view.view.view.layer.scroll_offset();
        let new_scroll_offset = Point::new(scroll_pos.x, scroll_pos.y - 100);
    ...
    fn render_structured_message_with_attributes(
        ...
        let rendered_leading_text_size =
            rendered_string_size(attributes.leading_text, &self.font_italic, font_size);

        // Figure out the layout of the 'content' / main text
        let start_of_message_content_x =
    ...
    fn render_structured_server_notice(&self, leading_text: &str, message_text: &str) {
        self.render_structured_message_with_attributes(RenderStructuredMessageAttributes::new(
            leading_text,
            Color::new(40, 40, 40),
    ...
    fn render_structured_user_notice(&self, leading_text: &str, message_text: &str) {
        self.render_structured_message_with_attributes(RenderStructuredMessageAttributes::new(
            leading_text,
            Color::new(20, 20, 20),
    ...
    fn render_structured_user_notice_level2(&self, leading_text: &str, message_text: &str) {
        self.render_structured_message_with_attributes(RenderStructuredMessageAttributes::new(
            leading_text,
            Color::black(),
            Color::new(255, 143, 38),
    ...
    fn render_message(&self, msg: IrcMessage) {
        match msg.command {
            ...
            IrcCommand::ReplyGlobalUsers(p) => {
                self.render_structured_server_notice("Stats", &p.message);
            }
            ...
            IrcCommand::TopicLastSet(p) => {
                self.render_structured_server_notice(
                    &format!("{} Topic Update History", p.channel),
                    &format!("Last updated by {}", p.user),
                );
            }
            unknown => {
    ...
    fn render_window_to_display(&self, graphics_protocol: &mut ScopedProtocol<GraphicsOutput>) {
        ...
        let buf_as_blt_pixel = unsafe {
            ...
            Vec::from_raw_parts(buf_as_blt_pixels, len, capacity)
        };
            ...
            match maybe_key {
                ...
                Some(key) => {
                    let key_as_u16 = match key {
                        Key::Special(scancode) => scancode.0,
                        Key::Printable(char_u16) => char::from(char_u16) as _,
                    };
    ...
    fn handle_mouse_updates(&self, pointer: &mut Pointer, pointer_resolution: Point) {
        ...
        if let Some(pointer_updates) = pointer_updates {
            ...
            if !orig_is_left_click_down && is_left_click_down_now {
                ...
                self.window
                    .handle_mouse_left_click_down(updated_current_pointer_pos);
            } else if orig_is_left_click_down && !is_left_click_down_now {
                // We just exited a left click
                self.window
                    .handle_mouse_left_click_up(updated_current_pointer_pos);
            }
    ...
    fn draw_cursor(&self) {
        ...
        let cursor_frame = Rect::from_parts(*self.current_pointer_pos.borrow(), self.cursor_size);
        // Inner cursor
